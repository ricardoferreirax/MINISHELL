...
/*   Updated: 2025/12/18 12:39:46 by rmedeiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* static void	prepare_external_or_exit(t_pipe_pos pos, t_cmd *cmd,
		t_pipeline *pp)
{
	int	resolve_status;
	int	resolve_status;
...
static void	prepare_external_or_exit(t_pipe_pos pos, t_cmd *cmd, t_pipeline *pp)
{
	...
	resolve_status = is_external_resolved(cmd, pp->mini);
	if (resolve_status <= 0)
		prepare_external_error(pos, cmd, pp, resolve_status);
