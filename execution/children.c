...
/*   Updated: 2025/09/26 19:09:09 by rmedeiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (pp->prev_pipefd != -1) // se houver pipe anterior → ligamos STDIN ao prev_pipefd
    {
        if (safe_dup2_and_close(pp->prev_pipefd, STDIN_FILENO) != 0)
            exit(1);
        pp->prev_pipefd = -1;
    ...
    if (cmd->next)
    {
        if (safe_dup2_and_close(pp->pipefd[1], STDOUT_FILENO) != 0) // se houver próximo comando → ligamos STDOUT ao pipe atual
            exit(1);
        close(pp->pipefd[0]);
    }
}
